...
class dbCalendar extends Table implements iDatabase
{
    const CLASS_ID = 5;
    ...
    {
        if (Valid::vString(self::$classQueries['Insert'])) {
            $returnData = (
                self::qRun(
                    array(
    ...
    public static function Select(array $array = array(), string $type = ''): array
    {
        ...
        if (! empty($array) && Valid::vString(self::$classQueries['Select'][$type])) {
    	    if (
                (
                    $tempVar = self::qRun(
                            array(
