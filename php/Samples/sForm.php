...
class sForm
{
        ...
        if (is_array($constData) && is_array($staticData)) {
            foreach ($constData as $i) {
                if (stristr($i["tags"], $staticData["tag"])) {
                    ...
                    if (
                        (!strcmp($tempArray["tag"], "Edit") || !strcmp($tempArray["tag"], "View")) &&
                        is_array($valueData)
                    ) {
                        $tempArray["value"] = isset($valueData[$tempArray["data"]])
                            ? $valueData[$tempArray["data"]]
                            : "";
                    } else {
    ...
    public static function Input(array $array): string
    {
        $returnString = $input = "";
        $array["inputId"] =
            isset($array["origo"]) && isset($array["data"]) ? $array["origo"] . "-" . $array["data"] : "";
        ...
        $array["input"] =
            '<input type="' .
            $array["type"] .
            '" class="form-control col-12 p-0 m-0" id="' .
            $array["inputId"] .
            '" name="' .
            $array["data"] .
            '" value="' .
            ...
            '"' .
            $array["disabled"] .
            ($array["mustFill"] ? ' data-must-fill="true"' : "") .
            ">";
    ...
    private static function Input_Select(array $array): string
    {
        ...
        foreach ($array["select"] as $i) {
            $returnString .=
                ...
                $i["id"] .
                '"' .
                (!strcmp((string) $array["value"], (string) $i["id"]) ? " selected" : "") .
                ">" .
                $i["name"] .
                "</option>";
        }
        ...
        return empty($array["disabled"])
            ? '
				<div class="form-group col-12 mx-0">
					<textarea class="ckeditor col-12" id="' .
                    $array["inputId"] .
                    '">' .
            ...
            : '
				<div ' .
                    (isset($array["print"]) && $array["print"] ? 'id="printThis" ' : "") .
                    'class="border-top border-bottom form-group col col-12 mx-0">
					' .
                    $array["value"] .
                    '
				</div>';
    ...
    private static function Input_Default(array $array): string
    {
        return '
			<div class="form-group row col-12 mx-0">
				<label for="' .
            $array["inputId"] .
            '" class="col-sm-6 col-form-label text-left">' .
            $array["desc"] .
            ($array["mustFill"] && !isset($array["disabled"]) ? " *" : "") .
            '</label>
    ...
    public static function Spinner(array $array): string
    {
        if (Valid::vArray($array)) {
            ...
            if (isset($array["x"]) && !empty($array["x"])) {
                $location .= "x=" . $array["x"];
                if (isset($array["y"]) && !empty($array["y"])) {
                    $location .= "&y=" . $array["y"];
                    if (isset($array["z"]) && !empty($array["z"])) {
                        $location .= "&z=" . $array["z"];
                    }
                }
            } else {
            ...
            return '
                ...
                $GLOBALS["Root"]["Path"] .
                'images/placeholder.png" alt="Loading Animation" onload="setTimeout( function(){ Instance.initElmnt(\'' .
                $location .
                '\'); $(\'[data-dismiss=modal]\').trigger({ type: \'click\' }); ' .
                (isset($array["reload"]) && $array["reload"] ? "window.location.reload();" : "") .
                ' }, 750);">
                    </div>
                </div>';
        } else {
        ...
        if (isset($array["path"])) {
            return '
                <div class="ml-sm-auto">
                    <a class="col-12 btn btn-success" role="button" data-link="' .
                $array["path"] .
                '/Add"' .
                ($array["addNonModal"] === true ? "" : ' data-post="0" data-toggle="modal" data-target="#modalBox"') .
                '>
                        <i class="fa fa-' .
                $array["fa"] .
                '" aria-hidden="true"></i>
    ...
    public static function Button(array $array): string
    {
        if (isset($array["path"])) {
            return '
                ...
                $array["path"] .
                '" data-post="' .
                (isset($array["dp"]) ? $array["dp"] : "") .
                '"' .
                (isset($array["nonModal"]) && $array["nonModal"]
                    ? ""
    ...
    public static function Download(array $array = []): string
    {
            ...
            if (file_exists(substr_replace($GLOBALS["Root"]["Site"], "", -1) . $f[$array["filePath"]])) {
                $Prompt_Body .=
            		...
            		<div class="form-group row col-12 my-1 mx-0">
            		    <a class="btn btn-primary col-12" href="/php/Download.php?fs=' .
                    $f[$array["filePath"]] .
                    "&fn=" .
                    $f[$array["fileName"]] .
                    "&ft=" .
                    ...
                    '
                    				    ...
                    				    <td class="w-25 float-start">
                    				        ' .
                    round((int) $f[$array["fileSize"]] / 1024, 2) .
                    ' kB
                    				    </td>
                    				</tr>
                			    </tbody>
                ...
                if (isset($f[$array["fileId"]])) {
                    $wClass->Delete([
                        $array["fileId"] => (int) $f[$array["fileId"]],
