            ...
            if (Valid::vInput($array["userContEmail"], 4) && Valid::vInput($array["userName"], 0)) {
                if (empty(self::Select($array, "byUserName")) && empty(self::Select($array, "byContEmail"))) {
                    ...
                    if ($bool_1) {
                        ...
                        $addHuntgroup = $dHuntgroup::Insert([
                            "huntgroupName" => "#_" . $array["userName"],
                            "huntgroupDesc" => "Felhasználóra vonatkozó egyedi csoport",
                            "isDelete" => 0,
                        ]);

                        $selectUserId = self::Select($array, "byUserName")[0]["userId"];
                        $selectHuntgroupId = $dHuntgroup::Select(
                            ["huntgroupName" => "#_" . $array["userName"]],
                            "byHuntgroupName_Last"
                        )[0]["huntgroupId"];

                        $addGroup_Member = $dGroup_Member::Insert([
                            "userId" => (int) $selectUserId,
                            "groupId" => (int) $GLOBALS["Data"]["groupDefault"],
                        ]);
                        $addHuntgroup_Member_to_Self = $dHuntgroup_Member::Insert([
                            "userId" => (int) $selectUserId,
                            "huntgroupId" => $selectHuntgroupId,
                        ]);
                        $addHuntgroup_Member_to_Default = $dHuntgroup_Member::Insert([
                            "userId" => (int) $selectUserId,
                            "huntgroupId" => (int) $GLOBALS["Data"]["huntgroupDefault"],
                        ]);
        ...
        switch ($type) {
            ...
            case "Password":
                ...
                if (
                    ...
                    return self::$dbUser->Update(
                        ["userId" => $array["userId"], "pWord" => md5($array["userPassword_New"])],
                        $type
                    );
                } else {
                    return false;
            ...
            case "Reset":
                return self::$dbUser->Update(["userId" => $array["userId"], "pWord" => $array["pWord"]], "Password");
            default:
                return false;
        }
    }
